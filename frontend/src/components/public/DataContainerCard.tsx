...
const schoolAdmissionRates: Record<string, {
  name: string;
  minScore: number;
  rate: (score: number) => number;
}> = {
  unilag: {
      ...
      <div className="relative w-full">
        ...
        <div className="relative w-full max-w-md bg-primary-900/20 dark:bg-gray-900/25 backdrop-blur-xl border border-white/10 dark:border-white/15 rounded-2xl shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">ADMISSION CALCULATOR</h3>
          ...
          <div className="p-4">
            <div className="bg-gray-800/20 dark:bg-gray-800/30 border border-white/5 rounded-xl p-4">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs text-primary-300/80 mb-1.5 flex items-center gap-1">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-3 py-1.5 text-xs bg-gray-700/20 border border-gray-600/30 rounded-xl text-gray-300 focus:outline-none focus:ring-1 focus:ring-accent-400/30 appearance-none cursor-pointer pointer-events-auto transition-colors"
                    >
                  ...
                  <div className="mt-1 text-[10px] text-primary-400/50">
                    Min. required: {school.minScore}
                  </div>
                </div>

                <div>
                  <div className="flex justify-between text-xs text-primary-300/80 mb-1.5">
                    <span>JAMB Score: {jambScore}</span>
                    <span>Cut-off: {school.minScore}</span>
                  </div>
                  <input
                    type="range"
                    min="150"
                    ...
                    className="w-full h-2 bg-gray-600/30 rounded-full accent-accent-400 cursor-pointer pointer-events-auto"
                  />
              ...
              <div className="mt-4 text-center">
                ...
                <span className="text-xs text-primary-300/80 block">
                  Chance of Admission at {school.name.split('(')[0].trim()}
                </span>
              </div>
            </div>

            <div className="mt-3 text-xs text-center text-primary-300/60">
